        ...
        <div className='w-full'>
            <div className='flex flex-col md:flex-row items-center bg-white border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-full p-4 transition-all hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]'>
                ...
                <div className='flex flex-col flex-1 px-8 py-2 text-left hover:bg-slate-50 rounded-full transition-colors cursor-pointer group'>
                    <label className='text-[10px] uppercase font-bold text-blue-600 tracking-widest mb-0.5'>Location</label>
                    <select 
                        required 
                ...
                <div className='flex flex-col flex-1 px-8 py-2 text-left hover:bg-slate-50 rounded-full transition-colors cursor-pointer'>
                    ...
                    <input 
                        type='date'
                        className='bg-transparent outline-none text-[14px] text-slate-600 font-medium cursor-pointer w-full' min={new Date().toISOString().split('T')[0]}
                    />
                </div>
                ...
                <button className='flex items-center justify-center gap-3 bg-gradient-to-r from-blue-600 to-blue-500 hover:to-blue-400 text-white px-10 py-4 rounded-full transition-all active:scale-95 shadow-lg shadow-blue-200 ml-2'>
            ...
            <div className='mt-4 flex justify-center'>
                {!pickuplocation ? (
                    <p className='text-[12px] text-slate-400 font-medium tracking-tight'>Choose your destination to see available luxury fleet</p>
                ) : (
                    <p className='text-[12px] text-blue-600 font-bold uppercase tracking-widest animate-pulse'>Available in {pickuplocation}</p>
                )}
            </div>
        </div>
    )

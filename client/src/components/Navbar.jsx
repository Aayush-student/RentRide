...
const Navbar = () => {
    ...
    const changeRole = async () => {
        ...
        try {
            const { data } = await axios.patch('/api/owner/change-role', {}, {
                headers: {
            ...
            const message = error.response?.data?.message || "Not Authorized"
            toast.error(message)
        }
    }

    return (
        <nav className="fixed top-0 left-0 w-full bg-white/95 backdrop-blur-md border-b border-gray-100 px-4 md:px-12 py-1 z-50 transition-all duration-300 hover:shadow-xl hover:shadow-blue-900/5 font-['Outfit']">
            <div className="w-full flex items-center justify-between max-w-7xl mx-auto">
                
                <Link to="/" className="flex-shrink-0 transition-transform hover:scale-105 active:scale-95">
                    <img src={assets.bikelogo} alt="logo" className="h-16 md:h-20 w-auto object-contain" />
                </Link>
                ...
                `}>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        >
                            {isOwner ? "Dashboard" : "List your Bike"}
                        </button>
                        
                        <button 
                            className="cursor-pointer px-8 py-2.5 bg-gradient-to-r from-gray-900 to-blue-900 hover:from-black hover:to-blue-950 text-white rounded-lg text-[15px] font-medium tracking-wide shadow-lg shadow-blue-900/10 hover:shadow-blue-900/20 hover:-translate-y-0.5 transition-all duration-300 active:scale-95"
                            onClick={() => {
                                if (user) {
                                    logout();
                                } else {
